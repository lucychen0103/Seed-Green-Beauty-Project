"""EPA Grants Playwright scraper — government funding opportunities.

Scrapes current grant opportunities from epa.gov/grants.
...
GRANTS_URL = "https://www.epa.gov/grants/epa-grants-listings"
ROBOTS_URL = "https://www.epa.gov/robots.txt"
...
async def scrape(headless: bool = True) -> List[FundingRecord]:
    """Return current EPA grant opportunities."""
    ...
    async with async_playwright() as p:
        ...
        if not await _robots_allows(context):
            logger.warning("epa_grants: robots.txt disallows scraping — skipping")
            await browser.close()
            return records
        ...
        try:
            ...
            logger.error("epa_grants: scrape failed: %s", exc, exc_info=True)

        await browser.close()

    if not records:
        logger.warning("epa_grants: scraper returned 0 results")
...
async def _robots_allows(context: BrowserContext) -> bool:
    ...
    try:
        ...
        await page.close()
        return "Disallow: /grants" not in text
    except Exception:
        return True
...
async def _scrape_listings(context: BrowserContext) -> List[FundingRecord]:
    ...
    # EPA grants listing is a table or list of links — gather all grant links
    grant_links = await _collect_grant_links(page)
    logger.info("epa_grants: found %d grant links", len(grant_links))
    ...
    for href in grant_links:
        try:
            ...
            logger.warning("epa_grants: failed to scrape %s: %s", href, exc)
...
async def _collect_grant_links(page: Page) -> List[str]:
    """Return absolute URLs of individual grant opportunity pages."""
    hrefs: List[str] = []

    # EPA grants listing uses a table with links to individual grant pages
    anchors = await page.query_selector_all(
        "table a[href], .view-content a[href], .field-content a[href]"
    )
    for anchor in anchors:
        href = await anchor.get_attribute("href")
        if not href:
            continue
        # Keep only links that look like grant opportunity pages
        if "/grants/" in href or "/grant-opportunity/" in href:
...
async def _scrape_grant_page(
    ...
    """Scrape an individual EPA grant opportunity page."""
    page = await context.new_page()
    ...
    # Extract body text for description and deadline detection
